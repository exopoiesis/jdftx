...
protected:
	...
	virtual void offloadVariable(Variable&) const {} //!< Optional: move variable data to CPU after storing in history (for GPU memory savings)
...
template<typename Variable> double Pulay<Variable>::minimize(double Eprev, std::vector<string> extraNames, std::vector<double> extraThresh)
{
	...
	for(int iter=0; iter<pp.nIterations; iter++)
	{
		...
		for(size_t j=0; j<ndim; j++)
			alpha.push_back(cOverlap_inv(j, ndim).real());
